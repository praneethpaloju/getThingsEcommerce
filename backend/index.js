...
// Creating API to remove products
...
app.post("/signup", async (req, res) => {
  ...
  if (check) {
    return res
      .status(400)
      .json({ success: false, errors: "User exists with this email ID" });
  }
  let cart = {};
  for (let i = 0; i < 300; i++) {
    cart[i] = 0;
  ...
  const token = jwt.sign(
    data,
    "secret_ecom"
  ); /* secret_ecom- this is called as SALT.  This adds 1 layer of security */
...
app.post("/login", async (req, res) => {
  ...
  if (user) {
    ...
    res.json({ success: false, errors: "Wrong email id register if not" });
  }
});
...
app.get("/relatedproducts", async (req, res) => {
  let products = await Product.find({});
  let related_products = products.slice(1).slice(-4);
...
app.post("/addtocart", fetchUser, async (req, res) => {
  ...
  // console.log(`{${req.body.itemId}:${userData.cartData[req.body.itemId]}}`);
